...
| extract_provenance | TEXT nullable | JSON: per-field source (`description` \| `whop_page` \| `regex` \| `allowed_socials` \| `absent`) — honest coverage tracking; `allowed_socials` marks a field defaulted from A's ingested field, not read from brief text |
