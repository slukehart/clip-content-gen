...
from clipscore.db.models import Campaign, CampaignScore, ClipJob
from clipscore.web.actions import clip_this
from clipscore.web.queries import approval_rows


def _campaign(session, cid="c1", *, scored=False, **kwargs):
    """Add a clipping campaign. `scored=True` also adds a CampaignScore so it
    passes `eligible_latest_scores` (required for the approval board)."""
...
def test_approval_row_builds_whop_campaign_deep_link(session):
    _campaign(session, scored=True, title="WarHamster", url="https://whop.com/virality-37",
...
def test_approval_row_falls_back_to_stored_url_without_ids(session):
    _campaign(session, scored=True, title="Cool", url="https://whop.com/cool-campaign")
    rows = approval_rows(session, Settings(_env_file=None))
