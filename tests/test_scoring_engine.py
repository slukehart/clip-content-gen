...
def test_multi_niche_no_cross_contamination(session):
    # Discriminating isolation test: gaming has TWO campaigns spanning a cvs range,
    # music has ONE campaign whose cvs falls BETWEEN them (via a mid sat_factor).
    # Correct per-niche isolation -> music's sole campaign is top of its niche = 1.0.
    # A bug pooling all three into one population would drop it to ~2/3 (its middle
    # rank), and gaming's low campaign from 0.5 to ~1/3 -- so both asserts fail.
    _seed_refs(session)
    session.add(NicheBaseline(niche="music", e_views_median=8000, p_threshold=0.55))
    session.commit()
    # cvs scales with sat_factor here (cpm/baseline/budget_health identical):
    _campaign(session, "g_hi", niche="gaming", cpm_usd=2.0); _snap(session, "g_hi", 5000.0, clippers=1)   # sat 1.0
    _campaign(session, "g_lo", niche="gaming", cpm_usd=2.0); _snap(session, "g_lo", 100.0, clippers=50)    # sat 0.01
    _campaign(session, "m_mid", niche="music", cpm_usd=2.0); _snap(session, "m_mid", 1000.0, clippers=10)  # sat 0.5
    score_all(session)
    rows = {r.campaign_id: r for r in session.execute(select(CampaignScore)).scalars()}
    assert rows["m_mid"].cvs_niche_percentile == pytest.approx(1.0)   # pooled would be ~2/3
    assert rows["g_lo"].cvs_niche_percentile == pytest.approx(0.5)    # pooled would be ~1/3
    assert rows["g_hi"].cvs_niche_percentile == pytest.approx(1.0)
